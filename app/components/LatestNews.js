      ...
      <div className="ml-20 max-md:ml-1 ">
        {props.data.map((item, index) => {
          return (
            <div
              key={index}
              className="flex mb-10 max-md:flex-col border-b-4 max-w-[100%]"
            >
              ...
              <div className="max-w-[40%]  max-md:max-w-[100%] ">
                <Link href={`/more/${item._id}`}>
                  <h3 className="font-sans text-2xl font-bold antialiased mx-2 hover:underline">
                    {item.title}
                  </h3>
                </Link>
                <p className="line-clamp-3 font-sans text-xl antialiased mx-2">
                  {item.description > 100
                    ? item.description.substring(0, 100) + "..."
                    : item.description}
                </p>
                <p className="font-sans text-md antialiased mx-2 mt-1">
