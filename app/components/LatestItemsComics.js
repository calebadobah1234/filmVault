...
const LatestItems = (props) => {
  ...
  const getImagePath = (item) => {
    ...
    if (item.chapters && item.chapters.length > 0 && item.chapters[0].images && item.chapters[0].images.length > 0) {
      // Prefer r2_url if available, otherwise use original_src
          ...
          {data.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* For comics, show publisher or year */}
                  {item.chapters && item.chapters.length > 0 ? (
                    <p>{item.publisher || item.year || "Comic"}</p>
                  ) : (
                    item?.categories?.[0] && <p>{item.categories[0]}</p>
                  )}
                </div>

                {/* Show total chapters for comics, or IMDB rating for movies/series */}
                {item.chapters && item.chapters.length > 0 ? (
                  <div
                  ...
                  >
                    <p>
                      {item.totalChapters || item.chapters.length} Ch
                    </p>
                  </div>
                ) : (
