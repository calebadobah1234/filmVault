...
const EnhancedStreamingComponent = ({ sources }) => {
  ...
  const processVideo = async (url) => {
    try {
      ...
      const apiUrl = `http://139.162.174.197:3000/download?url=${encodedUrl}&filename=${encodedFilename}`;
  ...
  }, [controlsTimeout, hideControls, isFullscreen]);
  ...
  return (
    ...
    >
      <div className={`relative ${isFullscreen ? 'h-screen flex items-center justify-center' : 'pt-[56.25%]'}`}>
