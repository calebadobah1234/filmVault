...
export async function generateMetadata({ params }) {
  const res = await fetch(
    `https://filmvaultbackend-2-gftk.onrender.com/get-item-detailsAiome/${params.title}`,
    {
      cache: "force-cache",
    }
  );
  ...
  async function fetchRelatedContent(categories, actors, title) {
    try {
      ...
      const response = await fetch(
        `https://filmvaultbackend-2-gftk.onrender.com/get-related-contentAiome?${queryString}`,
