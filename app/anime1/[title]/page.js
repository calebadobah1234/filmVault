...
export async function generateMetadata({ params }) {
  const res = await fetch(
    `https://api3.mp3vault.xyz/get-item-detailsAiome/${params.title}`,
    {
      cache: "force-cache",
    }
  );
  ...
  async function fetchRelatedContent(categories, actors, title) {
    try {
      ...
      const response = await fetch(
        `https://api3.mp3vault.xyz/get-related-contentAiome?${queryString}`,
  ...
  return (
    <>
      ...
      <div className="container mx-auto p-4 ">
        ...
        {/* <EnhancedSeriesStreamingComponent seasons={data.seasons} movieTitle={data.title}/> */}
        ...
        <CommentSection itemId={data._id} linkIdentifier="Aiome" />
      </div>
    </>
  );
};
