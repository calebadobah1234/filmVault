...
export async function generateMetadata({ params }) {
  const res = await fetch(
    `https://filmvaultbackend-3-b6cs.onrender.com/get-item-detailsAiome/${params.title}`,
    {
      cache: "force-cache",
    }
  );
  ...
  async function fetchRelatedContent(categories, actors, title) {
    try {
      ...
      const response = await fetch(
        `https://filmvaultbackend-3-b6cs.onrender.com/get-related-contentAiome?${queryString}`,
