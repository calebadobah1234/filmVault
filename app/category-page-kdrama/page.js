...
export async function generateMetadata({ searchParams }) {
  ...
  return {
    title: `${category} Korean Series - Page ${currentPage}`,
    description: `Browse and download ${category} korean series for free on page ${currentPage} of our collection.`,
  };
}

const page = async ({ searchParams }) => {
  const res = await fetch(
    `https://filmvaultbackend-2-gftk.onrender.com/get-category-dataAiokd/?category=${searchParams.category}&limit=30&skip=${searchParams.skip}`,
      ...
      <div className="my-6 px-4">
        <h1 className="text-3xl font-bold text-center text-gray-800 leading-tight">
          <span className="block mb-2 capitalize">
            {searchParams.category} Korean Series
          </span>
