...
export async function generateMetadata({ params }) {
  const res = await fetch(
    `https://filmvaultbackend-xpgy.onrender.com/get-item-detailsAiom/${params.title}`,
    {
      cache: "force-cache",
    }
  );
...
const page = async ({ params }) => {
  const res = await fetch(
    `https://filmvaultbackend-4.onrender.com/get-item-detailsAiom/${params.title}`,
    {
      revalidate: 86400,
    }
  );
  ...
  async function fetchRelatedContent(categories, actors, title) {
    try {
      ...
      const response = await fetch(
        `https://filmvaultbackend-4.onrender.com/get-related-contentAiom?${queryString}`,
