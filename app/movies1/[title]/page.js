...
export async function generateMetadata({ params }) {
  const res = await fetch(
    `https://filmvaultbackend-1.onrender.com/get-item-detailsAm/${params.title}`,
    {
      cache: "force-cache",
    }
  );
...
const page = async ({ params }) => {
  ...
  async function fetchRelatedContent(categories, actors, title) {
    ...
    try {
      ...
      const url = `https://filmvaultbackend-1.onrender.com/get-related-contentAm?${queryString}`;
