const aioMovie = require("../models/moviePovie");
...
const get20ItemsAiom = async (req, res) => {
  try {
    const items = await aioMovie.find({}).limit(20).sort({ _id: -1 });
...
const getHighestRatedMoviesAiom = async (req, res) => {
  try {
    const items = await aioMovie.find({}).sort({ imdb: -1 }).limit(20);
    res.status(200).json(items);
  } catch (error) {
    handleServerError(res, error);
  }
...
const getItemDetailsAiom = async (req, res) => {
  try {
    const { title } = req.params;
    if (!title) {
      return res.status(400).json({ error: "Title is required" });
    }
    const item = await aioMovie.find({ title }).limit(1);
    if (item.length === 0) {
...
const findItemsAiom = async (req, res) => {
  try {
    ...
    const [items, totalCount] = await Promise.all([
      aioMovie.aggregate(pipeline),
      aioMovie.countDocuments({
        $or: [
          { title: { $regex: flexibleRegex } },
          { actors: containsRegex },
          { categories: containsRegex },
...
const getRelatedContentAiom = async (req, res) => {
  try {
    ...
    const items = await aioMovie.aggregate(pipeline);
...
const getCategoryDataAiom = async (req, res) => {
  try {
    ...
    if (category.toLowerCase() === "all") {
      ...
      const result = await aioMovie.aggregate(pipeline);
...
const getOtherActorMoviesAiom = async (req, res) => {
  try {
    const { actor } = req.params;
    if (!actor) {
      return res.status(400).json({ error: "Actor name is required" });
    }
    const items = await aioMovie
      .find({
...
const addViewsAiom = async (req, res) => {
  try {
    const { _id } = req.params;
    if (!_id) {
      return res.status(400).json({ error: "ID is required" });
    }
    const item = await aioMovie.findByIdAndUpdate(
      _id,
      { $inc: { views: 1 } },
      { new: true }
    );
...
const sortViewsAiom = async (req, res) => {
  try {
    const items = await aioMovie.find({}).limit(12).sort({ views: -1 });
    res.status(200).json(items);
  } catch (error) {
    handleServerError(res, error);
  }
...
const getPaginateContentAiom = async (req, res) => {
  try {
...
const getMoviesByYearAiom = async (req, res) => {
  try {
    ...
    const totalCount = await aioMovie.countDocuments(query);
...
const increaseSearchPriorityAiom = async (req, res) => {
  try {
    ...
    const increase = await aioMovie.findOneAndUpdate(
      { title: title },
